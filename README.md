...
```
[1/5] P4 서버 설정
  P4 서버 주소: perforce.example.com:1666
  P4 계정: CODE
  P4 비밀번호: ****
  P4 Stream: //depot/dev
  연결 테스트 중... OK (virtual stream 감지: parent=//depot/main)
...
```bash
P4GITSYNC_P4_PORT=perforce.example.com:1666
...
def p4_to_git(changelist_info: dict) -> dict:
    ...
    ws = changelist_info["workspace"]          # "WORKSPACE_alice"
    m = re.match(r"CODE_(.+)\.dev", ws)
    user_id = m.group(1) if m else "unknown"

    desc = changelist_info["description"]      # "[Alice Kim] 번역 개선"
    m2 = re.match(r"\[(.+?)\]", desc)
